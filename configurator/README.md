...
| `src/engine/pins.ts` | Lock pins of the frame: which hole of the support a pin follows, which side it goes in from, and where a panel corner bracket makes it an extended one (pure, tested) |
...
The 3D preview uses the part meshes exported by `site/scripts/export-parts.mjs` (served under `/parts/`; the standalone app serves `../site/public` too). Every connector is placed with the rotation from `orientation.ts`, lock pins sit in every occupied arm the way the geometry rules describe, feet plug into the floor arms. Panels are parametric in two dimensions, so each one is assembled the way `panel()` in `panel.scad` assembles it: a plate (inter-fit turned around, lying against the inside of the frame with its mounts reaching back out to the plane of the opening; full cover overlapping the supports by half a unit and standing one connector wall proud of them so it covers the connectors) plus real exported meshes of the support mount plate for that edge length and the corner bracket, placed with the same spins as the OpenSCAD module. The kit is exported from `site/scripts/scad/panel_kit.scad`. Without the mesh library the preview falls back to schematic boxes.
