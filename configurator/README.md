...
The 3D preview uses the part meshes exported by `site/scripts/export-parts.mjs` (served under `/parts/`; the standalone app serves `../site/public` too). Every connector is placed with the rotation from `orientation.ts`, lock pins sit in every occupied arm, pushed in from the inside of the rack along the hole axis of that support (vertical for beams, front to back for posts), feet plug into the floor arms. Panels are parametric in two dimensions, so each one is assembled the way `panel()` in `panel.scad` assembles it: a plate (inset and flush for inter-fit, overlapping the supports by half a unit for full cover, standing one connector wall proud of them so it covers the connectors) plus real exported meshes of the support mount plate for that edge length and the corner bracket, placed with the same spins as the OpenSCAD module. The kit is exported from `site/scripts/scad/panel_kit.scad`. Without the mesh library the preview falls back to schematic boxes.
